...
itemRoutes.delete(
  ...
  auth.authenticateUser,
  auth.authorizeRoles(CFG.ROLES.USER),
  itemController.deleteItemByUser
);
 ...
 *                   example: "Unauthorized"
 */
itemRoutes.get("/items/total", itemController.getTotalItem);
 ...
 *   patch:
 *     tags: [Items]
 *     summary: Approve or reject an item via CMS
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
 ...
 *               rejection_reason:
 *                 type: string
 *                 description: Reason for rejecting the item (optional, required if `approved` is false)
 ...
 *                 message:
 *                   type: string
 *                   example: "Item approval status updated successfully"
 *                 data:
 *                   type: object
 *                   properties:
 *                     id:
 ...
 *                       message:
 *                         type: string
 *                         example: "The 'approved' field is required"
 *       401:
 *         description: Unauthorized - User is not authenticated
 *         content:
 *           application/json:
...
itemRoutes.patch(
  "/admin/items/:id/approval",
  auth.authorizeRoles(CFG.ROLES.ADMIN),
  itemController.approveOrRejectItemCMS
);
